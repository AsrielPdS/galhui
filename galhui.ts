...
export function close(click?: click) {
  return g("span", `_ ${C.close}`, icon(icons.close)).on("click", click);
}
/**cancel button */
...
class FileInput<T extends IFileSelector = IFileSelector> extends Component<T, { input: [FSValue[]], submit: [str[]]; }> {
  input: G<HTMLInputElement>;
...
class Camera extends Component<ICamera, { input: [str]; }>{
...
class MobImgSelector extends Component<IMobImgSelector, { input: [str] }>{
  view() {
    let
      i = this.p,
      bd = div("bd"),
    ...
    return this.bind(div(`_ ${C.mobile}-imgsel`, [
      input, bd,
